...
L0 扫描件 PDF
  ↓
L1 解析引擎与渲染参数（MinerU / Docling+rapidocr、images_scale）
  ↓
...
| 后端 | 关键配置 |
|------|----------|
| MinerU | `MINERU_MODEL_MODE=full`（Linux），mac 常回退 lite |
| Docling | `DOCLING_OCR_ENGINE=rapidocr`，**勿用 auto→ocrmac** |
| Docling | `DOCLING_IMAGES_SCALE=2.0`（页图放大，减轻漏行） |
| Docling | `DOCLING_FORCE_REPARSE=true` 改参数后重跑 |
...
DOCLING_FORCE_REPARSE=true   # 仅改 L1 参数时
